...
contract RewardCenter {
  ...
  function evaluatePlanDeprecation(address payable planAddr) private returns (bool) {
    if (planRegistry[planAddr].balance <= 0 || planAddr.balance <= MIN_PLAN_ETH_BALANCE) {
  ...
  /**
    ...
    */
  function createRewardPlan(uint256 creatorContribution, uint256 signPeriodDaysLimit) payable external {
    creatorContribution = creatorContribution * MILLIETHER_TO_WEI;
    require(msg.value >= MIN_CREATOR_COLLAB && msg.value >= creatorContribution, "Invalid collaboration amount");
    require(signPeriodDaysLimit >= 0 && signPeriodDaysLimit <= MAX_SIGN_PERIOD_DAYS, "Invalid sign period time limit");

    if (!entityRegistry[msg.sender].active) {
      entityRegistry[msg.sender] = EntityProfile(true, msg.sender, 0);
    }
    require(entityRegistry[msg.sender].runningPlans < 255, "Running plans limit reached");
    entityRegistry[msg.sender].runningPlans++;

    RewardPlan plan = new RewardPlan(msg.sender, creatorContribution, signPeriodDaysLimit);
    entityRelatedPlans[msg.sender].push(address(plan));
    planRegistry[address(plan)] = PlanProfile(true, msg.sender, creatorContribution);
    require(payable(address(plan)).send(creatorContribution), "Payment failed");
  }

  function notifyFounderSigned(address founderAddress, uint256 collaborationAmount) external onlyPlans{
