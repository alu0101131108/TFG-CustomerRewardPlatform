...
enum Stages { CONSTRUCTION, SIGNING, ACTIVE, DEPRECATED }
...
contract RewardPlan {
  ...
  modifier onlyRewardCenter() {
    require(msg.sender == address(rewardCenter), "Only reward center authorized");
    _;
  }
  ...
  modifier signPeriodExpired() {
    require(block.timestamp > deployDate + signPeriodTimeLimit, "Sign period not expired");
    _;
  }

  constructor(address creator, uint256 creatorContribution, uint256 signPeriodDaysLimit_) {
  ...
  */
  ...
  function checkSpendRules(uint256 clientId) private {
    if (spendRules.length == 0 || spendRules[0].spends > clientSpendsRegistry[clientId].spends) return;
  ...
  */
  ...
  function addFounder(address addr, uint256 collaborationAmount) external atStage(Stages.CONSTRUCTION) onlyFounders {   //(256 founders max)
    founders.push(Founder(addr, collaborationAmount * MILLIETHER_TO_WEI, false));
  }

  function addNotifier(address addr) external atStage(Stages.CONSTRUCTION) onlyFounders { 
  ...
  function addSpendRule(uint256 spendsAmount, uint256 rewardAmount, bool sorted) external atStage(Stages.CONSTRUCTION) onlyFounders {
    if (sorted) {
      ...
      for (uint256 i = 0; i < spendRules.length; i++) {   // Index search, can be optimized.
  ...
  function sign() payable external atStages([Stages.CONSTRUCTION, Stages.SIGNING]) onlyFounders {
    ...
    // The other founders can pay to sign.
      ...
      for (uint8 i = 1; i < founders.length; i++) {
        if (msg.sender == founders[i].addr && !founders[i].signed) {
          ...
          rewardCenter.notifyFounderSigned(msg.sender, founders[i].collaborationAmount);
          break;
        }
      }      
    }
  ...
  function signPeriodExpiredRefund() external atStages([Stages.CONSTRUCTION, Stages.SIGNING]) signPeriodExpired onlyFounders {
    // If sign period has expired, return funds to every founder.
    for (uint8 i = 0; i < founders.length; i++) {
      if (founders[i].signed) {
        founders[i].signed = false;
        require(payable(founders[i].addr).send(founders[i].collaborationAmount), "Payment failed");
      }
    }
  ...
  /* ON ACTIVE */
  function signUpClient(uint256 clientId, address addr) external atStage(Stages.ACTIVE) onlyNotifiers {
    rewardCenter.signUpClient(clientId, addr);
  }

  function notifyItemsBought(uint256 clientId, ItemStack[] calldata items) external atStage(Stages.ACTIVE) onlyNotifiers {
    // Add items to the registry.
    // Check items reward rules logic.
  }

  function notifyAmountSpent(uint256 clientId, uint256 amount) external atStage(Stages.ACTIVE) onlyNotifiers {
